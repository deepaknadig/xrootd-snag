...
/*
 * FileSystem is all pass-through.
 */
                    ...
                    int   monid)
{
   ...
   if (chain_file)
   {
      ...
      return static_cast<XrdSfsFile*>(new File(user, monid, std::move(chain_file_ptr), m_eroute));
#else
      return static_cast<XrdSfsFile*>(new File(user, monid, chain_file_ptr, m_eroute));
#endif
   }
   return NULL;
}
